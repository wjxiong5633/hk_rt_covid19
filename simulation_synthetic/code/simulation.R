...
check_args <- function(
  ...
  ## edit by Weijia
  stopifnot(
    is.numeric(arnaught) && sum(arnaught > 0) == n_t +1 && (length(arnaught) == 1 || length(arnaught) == n_t + 1)
  )
  stopifnot(
...
# ## Test
# simulate_seir_stochastic(arnaught = 2.0, t_E = 4, t_I = 4, N = 1e6, S_init = 1e6-10, E_init = 0, I_init = 10, n_t = 1000, n_steps_per_t = 10) -> stochastic
# simulate_seir_ode(arnaught = 2.0, t_E = 4, t_I = 4, N = 1e6, S_init = 1e6-10, E_init = 0, I_init = 10, n_t = 1000, n_steps_per_t = 10) -> ode
# 
