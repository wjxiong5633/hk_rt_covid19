...
data_case <- as.matrix(data[,rep(4,4)]) ## 99 rows
  
for (id in 30:346){
  if(id< 150){
    data1 <- data_case[1:id,]
  }else{
    data1 <-  data_case[(id-150+1):id,]
  }
  ...
  # data_inc = inci_dat1
  # dat_final_1 <- data_inc
  # delay2 <- rint
  # delay2[,3] <- cumsum(delay2[,3])
  # 
  # 
  # for (i in 1:14){
  #   if (data_inc[nrow(data_inc)-i+1,3]>0){
  #     dat_final_1[nrow(data_inc)-i+1,3] <- data_inc[nrow(data_inc)-i+1,3] + rnbinom(1,data_inc[nrow(data_inc)-i+1,3],delay2[i,3])
  #     #data111[nrow(data_inc)-i+1,3] <- data_inc[nrow(data_inc)-i+1,3] + data_inc[nrow(data_inc)-i+1,3] * (1-delay2[i,3])/ delay2[i,3] ## mean = r*(1-p)/p
  #   }
  # }
  ...
  write.csv(z1,paste("./temp_wave3_150days/mcmc_summary_test_",id,".csv",sep=""),row.names = F)
}
