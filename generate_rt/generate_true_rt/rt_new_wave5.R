...
for (uuuu in 1:200){
  ...
  refdate <- dates("12/02/2021") 
  data <- read.csv("data_all_Dec.csv") %>% mutate(date = as.Date(date))  ## 
  data <- data[656:nrow(data),]   ## using 234: 440 Rt
  diff_days = dates(as.character(data$date[nrow(data)]),format = "y-m-d") - refdate  +1
  
  
  dt= data.frame(
    confirm.date = dates(as.character(data$date),format = "y-m-d") - refdate +1,
    type = "local",
  ...
  write.csv(z1,paste("./true_rt_wave5/mcmc_summary_test_",uuuu,"_",id,".csv",sep=""),row.names = F)
}
