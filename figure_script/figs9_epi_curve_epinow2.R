...
curve_plot2 = function(myperiod){
  ...
  arima_all = combine_data("arima") %>% 
    ...
    filter(proj == 0) 
  #myyear = ifelse(mywave == "Omicron","2022","2020-2021")
  ...
  df_long  = 
    ...
    mutate(
      ...
      model = factor(model, levels = c("TRUE","ARIMA","EpiNow2","RIDE","BP"))
    ) %>% 
    filter(model != "ARIMA1" & model!= "ENSEMBLE")
  ...
  if(myperiod != "train"){
    p = 
      ...
      theme( strip.background =element_rect(fill=ggplot2::alpha("darkblue",0.1)),
             ...
             plot.margin = ggplot2::margin(5,15,5,15,unit = "pt")
      )
  }else{
    p = 
      df%>% 
...
p_epinow2 = cowplot::plot_grid(curve_p,NULL, leg,
                         ncol = 1, rel_heights = c(15,-0.2, 0.6)) 

p_epinow2
ggpubr::ggexport(p_epinow2,filename = "./figure_result/figure_S9.pdf",width=10, height=8)
